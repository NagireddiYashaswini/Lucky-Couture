...
// Dark, boutique-styled centered card — a deliberately different palette
// and mood from the Login box (which uses a light theme) while both sit
    ...
    <div className="min-h-[calc(100vh-72px)] flex items-center justify-center px-5 py-16 bg-primary bg-[radial-gradient(circle_at_20%_10%,rgba(237,217,163,0.12),transparent_45%),radial-gradient(circle_at_80%_90%,rgba(206,160,126,0.15),transparent_45%)]">
      ...
      >
          ...
          <form onSubmit={submit} className="px-8 pb-8 flex flex-col gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 bg-bg/5 border border-bg/15 rounded-xl px-3.5 py-2.5 focus-within:border-highlight transition-colors">
                <User size={15} className="text-bg/50" />
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-transparent text-bg placeholder:text-bg/30 text-sm outline-none w-full" placeholder="Ananya Rao" />
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2 bg-bg/5 border border-bg/15 rounded-xl px-3.5 py-2.5 focus-within:border-highlight transition-colors">
                ...
                <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="bg-transparent text-bg placeholder:text-bg/30 text-sm outline-none w-full" placeholder="you@example.com" />
              </div>
            </div>
            <div>
              <label className="block text-xs text-bg/60 mb-1.5">Phone</label>
              <div className="flex items-center gap-2 bg-bg/5 border border-bg/15 rounded-xl px-3.5 py-2.5 focus-within:border-highlight transition-colors">
                <Phone size={15} className="text-bg/50" />
                <input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="bg-transparent text-bg placeholder:text-bg/30 text-sm outline-none w-full" placeholder="+91 98765 43210" />
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2 bg-bg/5 border border-bg/15 rounded-xl px-3.5 py-2.5 focus-within:border-highlight transition-colors">
                ...
                <input required type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="bg-transparent text-bg placeholder:text-bg/30 text-sm outline-none w-full" placeholder="••••••••" />
              </div>
            </div>
            <button type="submit" className="mt-2 w-full bg-highlight text-primary font-semibold py-3.5 rounded-full hover:bg-accent hover:text-white transition-colors shadow-card">
              Create Account
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-sm text-bg/60 mt-6">
          ...
          <Link to="/login" className="text-highlight font-semibold hover:underline">Log in</Link>
        </p>
      </motion.div>
    </div>
  );
